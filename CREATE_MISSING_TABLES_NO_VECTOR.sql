...
-- Note: pgvector extension is not available on Railway, so we'll create interview_questions without vector column for now

-- 1. Create analysis_results table (already exists, but ensure indexes exist)
...
CREATE INDEX IF NOT EXISTS idx_analysis_results_full_result ON analysis_results USING gin (full_result);

-- 2. Create interview_questions table WITHOUT vector column (pgvector not available on Railway)
CREATE TABLE IF NOT EXISTS interview_questions (
  ...
  -- embedding vector(1536),  -- Commented out: pgvector extension not available on Railway
