#!/bin/bash
# Run all migrations for redcube_content database using Railway CLI

export PATH="/opt/homebrew/opt/postgresql@16/bin:$PATH"

echo "🚀 Running migrations for redcube_content database using Railway CLI..."
echo ""

# List of migration files for redcube_content (in order)
MIGRATIONS=(
  "shared/database/init/07-phase4-scraper-tables.sql"
...
# Switch to redcube_content database first
echo "📦 Switching to redcube_content database..."
echo "\c redcube_content" | railway connect postgres
...
for migration in "${MIGRATIONS[@]}"; do
  if [ -f "$migration" ]; then
    ...
    # Pipe SQL file through Railway CLI
    cat "$migration" | railway connect postgres 2>&1 | grep -E "(CREATE|ALTER|ERROR|NOTICE|relation.*already exists)" | head -5
    echo ""
  else
...
echo ""
echo "Now verify tables were created:"
